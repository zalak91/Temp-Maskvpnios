            ...
            if url.pathExtension.lowercased() == "zip" {
                ...
                DispatchQueue.global(qos: .userInitiated).async {
                    ...
                    do {
                        ...
                        // test = String(fileContents.filter { !"\r".contains($0) })
                        //test = fileContents.trimmingCharacters(in: .whitespaces)

                        test = fileContents.replacingOccurrences(of: "\r", with: "", options: .literal, range: nil)
                    ...
                    let tunnelConfiguration = try? TunnelConfiguration(fromWgQuickConfig: test, called: fileBaseName)
                    DispatchQueue.main.async {
