...
class StatusMenu: NSMenu {
    ...
    let tunnelsManager: TunnelsManager?   // changed to optional for thekeyvpn

    var resVC: FirstMacOSViewController?

    var statusMenuItem: NSMenuItem?
    var networksMenuItem: NSMenuItem?
    var activateMenuItem: NSMenuItem?
    ...
    init(tunnelsManager: TunnelsManager) {
        ...
        // below line commented for thekeyvpn
...
/*
         ...
         addTunnelManagementItems()    // commented for thekeyvpn
         addItem(NSMenuItem.separator())
         addApplicationItems()    // commented for thekeyvpn
*/
        addApplicationItems()
    ...
    func addStatusMenuItems() {
        ...
        /////// below lines add for thekeyvpn

        let activateMenuItem = NSMenuItem(title: tr("macToggleStatusButtonActivate"), action: #selector(activateClicked(sender:)), keyEquivalent: "")
        activateMenuItem.target = self
        activateMenuItem.isHidden = false
        addItem(activateMenuItem)
        ...
        self.activateMenuItem = activateMenuItem  // line add for thekeyvpn
        self.deactivateMenuItem = deactivateMenuItem
    }

    func updateStatusMenuItems(with tunnel: TunnelContainer?) {
        guard let statusMenuItem = statusMenuItem, let networksMenuItem = networksMenuItem, let deactivateMenuItem = deactivateMenuItem, let activateMenuItem = activateMenuItem else { return }
        ...
        // below commented code for thekeyvpn
        ...
        */
        ...
        if tunnel.status == .active {
            activateMenuItem.isHidden = true
            deactivateMenuItem.isHidden = false
        } else {
            deactivateMenuItem.isHidden = true
            activateMenuItem.isHidden = false
        }
    ...
    @objc func deactivateClicked() {
        if let currentTunnel = currentTunnel {
          ...
          //  if(currentTunnel.status == )
            updateStatusMenuItems(with: currentTunnel)

        }
    }

    // below method added for thekeyvpn
    @objc func activateClicked(sender: AnyObject) {

        if let tunnelsManager = tunnelsManager {
            let tunnel = tunnelsManager.tunnel(named: "phone")!
            //print("tunnel name",tunnel.name)

            tunnelsManager.startActivation(of: tunnel)
            updateStatusMenuItems(with: tunnel)
...
class TunnelMenuItem: NSMenuItem {
    ...
    init(tunnel: TunnelContainer, action selector: Selector?) {
        ...
        //below commencted for thekeyvpn
