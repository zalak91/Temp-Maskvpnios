...
//    func apicall(){
//        let url = URL(string: "http://api.greensignalvpn.com/vpn/testapi")!
//
...
//    }
    ...
    private func startObservingTunnelStatuses() {
        ...
        statusObservationToken = NotificationCenter.default.addObserver(forName: .NEVPNStatusDidChange, object: nil, queue: OperationQueue.main) { [weak self] statusChangeNotification in
            ...
            #if os(OSX)
            print("under macos block")
            NotificationCenter.default.post(name: .tunnelStatus, object: tunnel.tunnelProvider.connection.status)
            #elseif os(iOS)
            print("under ios block")
            NotificationCenter.default.post(name: .tunnelStatus, object: tunnel.tunnelProvider.connection.status)
            #endif
